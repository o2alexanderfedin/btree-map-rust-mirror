...
pub(crate) extern "C" fn btree_malloc(size: u64) -> *mut () {
    ...
    unsafe {
        __builtin___memset_chk(ptr, 0, size, unsafe {
            __builtin_object_size(ptr as *const (), 0)
        })
    };
    return ptr;
}
...
pub(crate) extern "C" fn new_btree() -> *mut BtreeT {
    let bt: *mut BtreeT = btree_malloc(core::mem::size_of::<BtreeT>() as u64) as *mut BtreeT;
...
pub(crate) extern "C" fn new_node(
    key: *mut (),
    key_len: u64,
    value: *mut (),
    value_len: u64,
        ...
        __builtin___memcpy_chk(
            ...
            unsafe {
                __builtin_object_size(
                    unsafe { &raw mut (*node).p_key[0 as usize] } as *mut u8 as *const (),
                    0,
                )
            },
        )
    ...
    unsafe {
        __builtin___memcpy_chk(
            unsafe { (*node).value.value } as *mut (),
            value as *const (),
            unsafe { (*node).value.len },
            unsafe { __builtin_object_size(unsafe { (*node).value.value } as *const (), 0) },
        )
    };
    unsafe {
...
pub(crate) extern "C" fn find_value(
    self__1: &mut NodeT,
    key_hash: u32,
    key: *mut (),
    mut key_len: u64,
    ...
    if (*self__1).key_hash == key_hash {
        if unsafe {
            memcmp(
                &raw mut (*self__1).p_key[0 as usize] as *mut u8 as *const (),
                key as *const (),
                key_len,
            )
        } == 0
    ...
    if key_hash > (*self__1).key_hash {
        ...
        return find_value(
            unsafe { &mut *(*self__1).child_right },
            key_hash,
            key,
            key_len,
        );
...
pub(crate) extern "C" fn find_entry(self_: &BtreeT, key: *mut (), mut key_len: u64) -> *mut ValueT {
...
pub(crate) extern "C" fn add_node(self__1: &mut NodeT, n_node: *mut NodeT) -> () {
    ...
    if unsafe { (*n_node).key_hash } == (*self__1).key_hash {
        if unsafe {
            memcmp(
                &raw mut (*self__1).p_key[0 as usize] as *mut u8 as *const (),
                unsafe { &raw mut (*n_node).p_key[0 as usize] } as *mut u8 as *const (),
                unsafe { (*n_node).key_len },
            )
        } == 0
        {
            ...
            unsafe {
                __builtin___memcpy_chk(
                    (*self__1).value.value as *mut (),
                    unsafe { (*n_node).value.value } as *const (),
                    unsafe { (*n_node).value.len },
                    unsafe { __builtin_object_size((*self__1).value.value as *const (), 0) },
                )
            };
            free_node(n_node);
            return;
...
pub(crate) extern "C" fn add_entry(
    self_: &mut BtreeT,
    key: *mut (),
    key_len: u64,
    value: *mut (),
...
pub(crate) extern "C" fn delete_node(
    root: *mut NodeT,
    key_hash: u32,
    key: *mut (),
    mut key_len: u64,
    ...
    if key_hash < unsafe { (*root).key_hash } {
        ...
        unsafe {
            (*root).child_right =
                delete_node(unsafe { (*root).child_right }, key_hash, key, key_len)
        };
    } else {
        if unsafe {
            memcmp(
                unsafe { &raw mut (*root).p_key[0 as usize] } as *mut u8 as *const (),
                key as *const (),
                key_len,
            )
        } == 0
        {
            if unsafe { (*root).child_left } as *mut () == 0 as *mut () {
                ...
                return temp;
            } else if unsafe { (*root).child_right } as *mut () == 0 as *mut () {
            ...
            let mut temp: *mut NodeT = unsafe { (*root).child_right };
            while !(temp).is_null() && unsafe { (*temp).child_left } as *mut () != 0 as *mut () {
                temp = unsafe { (*temp).child_left };
            }

            /// Copy the inorder successor's content to this node
            unsafe {
                (*root).key_hash = unsafe { (*temp).key_hash }
            };
            unsafe {
                __builtin___memcpy_chk(
                    unsafe { &raw mut (*root).p_key[0 as usize] } as *mut u8 as *mut (),
                    unsafe { &raw mut (*temp).p_key[0 as usize] } as *mut u8 as *const (),
                    unsafe { (*temp).key_len },
                    unsafe {
                        __builtin_object_size(
                            unsafe { &raw mut (*root).p_key[0 as usize] } as *mut u8 as *const (),
            ...
            unsafe {
                __builtin___memcpy_chk(
                    unsafe { (*root).value.value } as *mut (),
                    unsafe { (*temp).value.value } as *const (),
                    unsafe { (*temp).value.len },
                    unsafe {
                        __builtin_object_size(unsafe { (*root).value.value } as *const (), 0)
                    },
                )
            };
            ...
            unsafe {
                (*root).child_right = delete_node(
                    unsafe { (*root).child_right },
                    unsafe { (*temp).key_hash },
                    unsafe { &raw mut (*temp).p_key[0 as usize] } as *mut u8 as *mut (),
                    unsafe { (*temp).key_len },
                )
            };
        }
...
pub(crate) extern "C" fn remove_entry(self_: *mut BtreeT, key: *mut (), mut key_len: u64) -> () {
...
pub(crate) extern "C" fn get_node_count(self__1: *mut NodeT) -> u64 {
    ...
    return (1 as u64)
        .wrapping_add(get_node_count(unsafe { (*self__1).child_left }))
        .wrapping_add(get_node_count(unsafe { (*self__1).child_right }));
}
...
pub(crate) extern "C" fn list_node_entries(self__1: *mut NodeT, list: *mut EntryListT) -> () {
    if self__1 as *mut () == 0 as *mut () {
        return;
    }
    list_node_entries(unsafe { (*self__1).child_left }, list);
    if unsafe { (*list).len } >= unsafe { (*list).cap } {
        return;
    }
    let entry: *mut EntryT = btree_malloc(core::mem::size_of::<EntryT>() as u64) as *mut EntryT;
    unsafe { (*entry).key.key = btree_malloc(unsafe { (*self__1).key_len }) as *mut u8 };
    unsafe { (*entry).key.len = unsafe { (*self__1).key_len } };
    unsafe {
        __builtin___memcpy_chk(
            unsafe { (*entry).key.key } as *mut (),
            unsafe { &raw mut (*self__1).p_key[0 as usize] } as *mut u8 as *const (),
            unsafe { (*self__1).key_len },
            unsafe { __builtin_object_size(unsafe { (*entry).key.key } as *const (), 0) },
        )
    };
    unsafe { (*entry).value.value = btree_malloc(unsafe { (*self__1).value.len }) as *mut u8 };
    unsafe { (*entry).value.len = unsafe { (*self__1).value.len } };
    unsafe {
        __builtin___memcpy_chk(
            unsafe { (*entry).value.value } as *mut (),
            unsafe { (*self__1).value.value } as *const (),
            unsafe { (*self__1).value.len },
            unsafe { __builtin_object_size(unsafe { (*entry).value.value } as *const (), 0) },
        )
    };
    unsafe {
...
pub(crate) extern "C" fn list_entries(self_: *mut BtreeT) -> *mut EntryListT {
    ...
    let list: *mut EntryListT =
        btree_malloc(core::mem::size_of::<EntryListT>() as u64) as *mut EntryListT;
    unsafe { (*list).len = 0 as u64 };
    unsafe { (*list).cap = get_entry_count(self_) };
    unsafe {
        (*list).entries = btree_malloc(
            (core::mem::size_of::<EntryT>() as u64).wrapping_mul(unsafe { (*list).cap }),
        ) as *mut EntryT
    };
...
pub(crate) extern "C" fn free_entry_list(list: &mut *mut EntryListT) -> () {
    {
        let mut i: u64 = 0 as u64;
        '__b2: loop {
            if !(i < unsafe { (**list).len }) {
                break '__b2;
            }
            '__c2: loop {
                btree_free(
                    unsafe { (*unsafe { (**list).entries.add(i as usize) }).key.key } as *mut (),
                );
                btree_free(
                    unsafe { (*unsafe { (**list).entries.add(i as usize) }).value.value }
                        as *mut (),
                );
                break '__c2;
            }
